create or replace
package body PKG_RESERVATION is
  ...
  */
  ...
  function check_reservation(P_VEHICLE_NUMBER VARCHAR2) RETURN VARCHAR2 IS
    ...
    canceled RESERVATION.CANCELED%TYPE;
  BEGIN
    select RESERVATION_ID,RESERVATION_TO,CANCELED
    INTO foundAuthorisation,expiryDate,canceled  
    from (
        SELECT RESERVATION_ID,RESERVATION_TO,CANCELED
        FROM RESERVATION
    ...
    IF NVL(canceled,'N') = 'Y'  THEN
       RETURN '[INVALID,Authorisation canceled]';
    END IF;
  ...
  function check_reservation(P_RESERVATION_ID NUMBER) RETURN VARCHAR2 is
    ...
    canceled RESERVATION.CANCELED%TYPE;
  BEGIN
    SELECT RESERVATION_TO,CANCELED INTO expiryDate,canceled FROM RESERVATION 
    WHERE RESERVATION_ID = P_RESERVATION_ID;
  ...
  BEGIN
    ...
    update RESERVATION set canceled = 'Y' WHERE RESERVATION_ID = foundAuthorisation;
    update charges set canceled = 'Y' WHERE RESERVATION_ID = foundAuthorisation;
