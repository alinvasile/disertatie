...
class RolesController {
	...
	def list = {
		...
		[authorityList: Roles.list(params)]
	}
	 ...
	 */
	def show = {
		def authority = Roles.get(params.id)
		if (!authority) {
			flash.message = "Roles not found with id $params.id"
	...
	def delete = {
		...
		flash.message = "Roles $params.id deleted."
		redirect action: list
	}
